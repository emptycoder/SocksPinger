...
namespace Socks5
{
    public static class Socks5Proxy
    {
        ...
        private static byte[] ConstructAuthBuffer(string username, string password)
        {
            ...
            Array.Copy(Encoding.ASCII.GetBytes(username), 0, 
                credentials, 2, username.Length);
            credentials[username.Length + 2] = (byte)password.Length;
            Array.Copy(Encoding.ASCII.GetBytes(password), 0, 
                credentials, 2, password.Length);
        ...
        private static byte GetDestAddressType(string host)
        {
            ...
            {
                AddressFamily.InterNetwork => Socks5Constants.AddrtypeIpv4,
                AddressFamily.InterNetworkV6 => Socks5Constants.AddrtypeIpv6,
                // ReSharper disable once UseStringInterpolation
                _ => throw new Exception(string.Format(
                    "The host address {0} of type '{1}' is not a supported address type.\n" +
        ...
        private static byte[] GetDestPortBytes(int value) => new[]
        {
